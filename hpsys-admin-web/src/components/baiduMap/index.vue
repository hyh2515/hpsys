...
const initControlPlugin = () => {
	// 比例尺
	props.plugins.includes("BMap.ScaleControl") &&
		baiduMap.value.addControl(new BMapGL.ScaleControl());
	// 缩放
	props.plugins.includes("BMap.ZoomControl") &&
		baiduMap.value.addControl(new BMapGL.ZoomControl());
	// 定位
	props.plugins.includes("BMap.LocationControl") &&
		baiduMap.value.addControl(new BMapGL.LocationControl());
	// 3D控件
	props.plugins.includes("BMap.NavigationControl3D") &&
		baiduMap.value.addControl(new BMapGL.NavigationControl3D());
};
		...
		const marker = new BMapGL.Marker(
			...
			point,
			{
				...
				icon: new BMapGL.Icon(
					d.img,
					new BMapGL.Size(
						d.imgWidth ? d.imgWidth : 40,
						d.imgHeight ? d.imgHeight : 40
					)
				),
			}
		);
...
const renderPolyline = (dataArr, option = {}) => {
	dataArr.forEach((d) => {
		baiduMapPointArr.value.push(
			new BMapGL.Point(d.position[0], d.position[1])
		);
	});
...
const renderInfoWindow = (dataArr) => {
	dataArr.forEach((d) => {
		baiduMapInfoWindowObj.value[d.position] = new BMapGL.InfoWindow(
			d.content.join("<br>"),
			{
				title: d.title,
